...
import {LangChangeEvent, TranslateService, TranslationChangeEvent} from "../lib/TranslateService";
...
const beforeEach = jasmine.beforeEach;
const afterEach = jasmine.afterEach;
const spyOn = jasmine.spyOn;
...
describe('TranslateService', () => {
  ...
  it('should compile translations only once, even when subscribing to translations while translations are loading', /*async*/(() => {
    spyOn(translate.currentLoader, 'getTranslation').and.callFake(() => {
      return timer(1000).pipe(mapTo(of(translations)));
    });
